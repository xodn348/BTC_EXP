#!/usr/bin/env python3
"""
Deviation Threshold Analysis Plot
- G_t threshold analysis in R_t = 0 scenario
- Deviation occurs when G_t >= 0.17% * X_t
"""
...
# Calculate ratio_i (paper formula)
base_delay = 742  # ms
kappa = 26.40     # ms/MB
...
Bt_MB = 1.0
delta_sec = (base_delay + kappa * Bt_MB) / 1000.0
rho_honest = 1 - np.exp(-lambda_rate * delta_sec)
rho_dev = 1 - np.exp(-lambda_rate * (delta_sec + w_sec))
ratio_i = (rho_dev - rho_honest) / (1 - rho_dev)

print(f"ratio_i = {ratio_i:.6f} = {ratio_i*100:.4f}%")

# Split by halving
pre = block_data[block_data['height'] < 840000]
post = block_data[block_data['height'] >= 840000]
...
pre_thresh = ratio_i * pre_mean * 1e6 / 1000  # K sat
post_thresh = ratio_i * post_mean * 1e6 / 1000
...
threshold_line = ratio_i * x_range * 1e6 / 1000  # K sat

# Threshold line
ax.plot(x_range, threshold_line, '-', color=COLOR_THRESHOLD_LINE, linewidth=3, 
        label=f'Threshold = 0.17% × $X_t$')
...
# Region shading
ax.fill_between(x_range, threshold_line, Y_MAX, alpha=0.35, color='#E74C3C')  # Red
ax.fill_between(x_range, 0, threshold_line, alpha=0.35, color='#3498DB')  # Blue
...
print("\n" + "=" * 70)
print("Deviation Threshold Summary")
print("=" * 70)
print(f"Deviation condition: G_t >= {ratio_i*100:.4f}% × X_t")
print()
