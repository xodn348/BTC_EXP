#!/usr/bin/env python3
"""
Policy Effect Analysis and Visualization
...
# Results file path (relative to project root from sim/ folder)
...
def plot_policy_config_and_effect(df):
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "policy_config_and_effect.pdf", dpi=300, bbox_inches='tight')
    print(f"✓ Saved: {OUTPUT_DIR / 'policy_config_and_effect.pdf'}")
    plt.close()
...
def plot_fee_floor_effect(df):
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "fee_floor_effect.pdf", dpi=300, bbox_inches='tight')
    print(f"✓ Saved: {OUTPUT_DIR / 'fee_floor_effect.pdf'}")
    plt.close()
...
def generate_latex_table(df):
    ...
    print("\n" + "=" * 80)
    print("LaTeX Table: Policy Configuration and Effect")
    print("=" * 80)
    
    print(r"""
\begin{table}[htbp]
\centering
\caption{Policy Configuration and Deviation Rate at G\_ratio = 0.17\%}
\label{tab:policy_config}
...
def main():
    print("=" * 80)
    print("Policy Effect Analysis and Visualization")
    print("=" * 80)
    ...
    print("=" * 80)
    print(f"\nGenerated files:")
    print(f"  - {OUTPUT_DIR / 'policy_config_and_effect.pdf'}")
    print(f"  - {OUTPUT_DIR / 'fee_floor_effect.pdf'}")
