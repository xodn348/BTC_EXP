...
class Miner:
    ...
    def __init__(self, miner_id, share, cost):
        ...
        self.cost = cost    # C_i: cost rate (sat/sec) - fallback value
...
def run_once(
    ...
    # Convert daily cost to per-second cost (time-based cost model)
    # This addresses the reviewer comment about random round lengths:
    # cost is now proportional to actual time elapsed between blocks (dt_sec)
    SECONDS_PER_DAY = 86400
    daily_costs = daily_costs.copy()
    daily_costs['cost_usd_per_sec'] = daily_costs['cost_usd_per_day'] / SECONDS_PER_DAY
    
    # Create (date, miner_id) to cost rate mapping (using each date's BTC price)
    cost_map = {}  # {(date, miner_id): cost_sat_per_sec}
    ...
    for _, row in daily_costs.iterrows():
        ...
        cost_sat_per_sec = (row['cost_usd_per_sec'] / btc_price) * 100_000_000
        key = (date_str, row['miner_id'])
        cost_map[key] = cost_sat_per_sec
    ...
    for t in range(T):
        ...
        # Calculate dt_sec: actual time elapsed since previous block (addresses reviewer comment)
        # This makes cost proportional to real time, not fixed per-block
        ts = int(block_row['timestamp'])
        if t == 0:
            dt_sec = 600.0  # First block uses average block time
        else:
            idx_prev = (idx - 1) % len(block_data)
            ts_prev = int(block_data.iloc[idx_prev]['timestamp'])
            dt_sec = max(1.0, ts - ts_prev)  # Clamp to minimum 1 second
        ...
        # 5) Per-miner costs (time-based: cost_rate * dt_sec)
        # cost_map contains sat/sec, multiply by dt_sec to get actual cost for this block interval
        cost_rates = np.array([cost_map.get((block_date, mid), costs[i]) for i, mid in enumerate(miner_ids)])
        block_costs = cost_rates * dt_sec  # sat/sec * sec = sat
...
def main():
    ...
    # Now using per-second rate (consistent with time-based cost model)
    ...
    SECONDS_PER_DAY = 86400
    # Fallback costs are per-second rates (will be multiplied by dt_sec in simulation)
    costs = np.array([(avg_costs.get(mid, 0) / SECONDS_PER_DAY / avg_btc_price * 100_000_000) for mid in miner_ids])
    ...
    try:
        ...
        # Additional columns needed (height, btc_usd, miner_id, timestamp)
        additional_cols = []
        ...
        if 'timestamp' in block_data.columns:
            additional_cols.append('timestamp')
